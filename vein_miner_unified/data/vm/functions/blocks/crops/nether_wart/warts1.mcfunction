...
summon area_effect_cloud ~ ~ ~ {Tags:["vm_wart"],Age:1}
execute store result score #vm_fortrand vm_calc run data get entity @e[type=area_effect_cloud,tag=vm_wart,distance=..0.01,limit=1] UUIDMost 0.00000000023283064
kill @e[type=area_effect_cloud,tag=vm_wart,distance=..0.01,limit=1]
...
execute if score @s vm_calc < #vm_cycle vm_pickus run function vm:blocks/crops/nether_wart/warts1